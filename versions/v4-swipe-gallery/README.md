# Ruth Talia Couture v4

A mobile-first, bilingual storefront with native touch carousels, in which the backend owns the catalog, multi-image products, carts, orders, Ruth's availability calendar, appointment requests, and page configuration.
...
```bash
cd /Users/yakovelmaleh/ruth-talia-couture-website/versions/v4-swipe-gallery
npm install
...
Edit the `gallery` array in `config/site.json`. Each entry has an `imageUrl` and bilingual `caption`. Images appear side by side in a smooth horizontal carousel. Visitors can swipe directly with a finger on mobile, use a trackpad, or select the arrows on desktop.
...
- Native touch swipe, scroll snapping, arrows, and dots for product images
- Side-by-side editorial gallery with native touch/trackpad swipe
...
1. Copy this whole `v4-swipe-gallery` folder into a new project/version folder.
