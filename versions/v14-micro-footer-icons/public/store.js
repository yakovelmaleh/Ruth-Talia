(() => {
  ...
  class InfiniteCarousel {
    constructor(track, onChange = () => {}) {
      ...
      this.viewport.addEventListener("pointerleave", event => {
        if (this.dragging && !this.pointerCaptured) this.endDrag(event);
      }, { signal: this.controller.signal });
    ...
    render(animate = true) {
      ...
      if (animate) {
        this.animating = true;
        this.transitionTimer = setTimeout(() => this.finishMove(), 520);
      } else {
        this.animating = false;
    ...
    startDrag(event) {
      ...
      if (event.target.closest(".card-arrow,.gallery-arrow,.media-arrow,.media-dots button") || this.animating) return;
      this.dragging = true;
  ...
  document.querySelectorAll("[data-card-carousel]").forEach(container => {
    ...
    [previous, next].forEach(button => button.addEventListener("pointerdown", event => event.stopPropagation()));
    ...
    container.querySelector("[data-product-card-open]").addEventListener("click", event => {
      if (Date.now() < carousel.suppressClickUntil) {
        event.preventDefault();
        return;
      }
