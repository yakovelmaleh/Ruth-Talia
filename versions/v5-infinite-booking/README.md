# Ruth Talia Couture v5

A mobile-first, bilingual storefront with seamless circular touch carousels. The backend owns the catalog, multi-image products, carts, orders, Ruth's availability calendar, appointment requests, and page configuration.
...
```bash
cd /Users/yakovelmaleh/ruth-talia-couture-website/versions/v5-infinite-booking
npm install
...
Edit the `gallery` array in `config/site.json`. Each entry has an `imageUrl` and bilingual `caption`. Images appear side by side in a seamless circular carousel. At the last image, the next swipe continues naturally to the first; the same behavior works in reverse.
...
- Infinite native touch swipe, scroll snapping, soft in-image chevrons, and dots for product images
- Infinite side-by-side editorial gallery with touch, trackpad, and in-image chevrons
...
1. Copy this whole `v5-infinite-booking` folder into a new project/version folder.
