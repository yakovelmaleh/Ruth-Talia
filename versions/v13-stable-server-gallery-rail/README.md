...
Version 13 retains and explicitly references the HTTP server so `npm start` remains attached to the terminal until `Control+C`. Footer contact icons are reduced to a subtle 11px size, and the Gallery now uses a reusable soft draggable rail matching the product collection.
...
```bash
cd /Users/yakovelmaleh/ruth-talia-couture-website/versions/v13-stable-server-gallery-rail
npm install
...
1. Copy this whole `v13-stable-server-gallery-rail` folder into a new project/version folder.
