...
function createDatabase(rootDir) {
  ...
  const imageUrls = input => {
    const values = String(input.imageUrls || input.imageUrl || "")
      ...
      .filter(Boolean);
    if (!values.length) throw new Error("At least one product image is required.");
    return values;
  };
  ...
  return {
    ...
    createAppointment(input) {
      const slotId = input.slotId ? Number(input.slotId) : null;
      db.exec("BEGIN IMMEDIATE");
      try {
        let slot;
        if (slotId) {
          slot = db.prepare("SELECT * FROM availability_slots WHERE id = ? AND active = 1 AND booked_at IS NULL").get(slotId);
          if (!slot) {
    ...
    listAvailableSlots() {
      ...
      `).all(new Date().toISOString().slice(0, 16));
    },
    listSlots() {
      return db.prepare("SELECT * FROM availability_slots ORDER BY starts_at DESC LIMIT 200").all();
    },
    createSlot(input) {
