...
app.post("/admin/slots", adminAuth, (req, res, next) => {
  try {
    requireFields(req.body, ["date", "time"]);
    store.createSlot(req.body);
    res.redirect("/admin#calendar");
  } catch (error) {
    next(error);
  }
});
app.post("/admin/slots/:id/delete", adminAuth, (req, res, next) => {
  try {
    store.deleteSlot(Number(req.params.id));
    res.redirect("/admin#calendar");
  } catch (error) {
    next(error);
  }
