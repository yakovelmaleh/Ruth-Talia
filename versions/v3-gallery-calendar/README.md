# Ruth Talia Couture v3

A mobile-first, bilingual storefront in which the backend owns the catalog, multi-image products, carts, orders, Ruth's availability calendar, appointment requests, and page configuration.
...
```bash
cd /Users/yakovelmaleh/ruth-talia-couture-website/versions/v3-gallery-calendar
npm install
...
5. Add one image URL per line. The first is the catalog cover; all images appear in the swipe gallery.
...
Edit the `gallery` array in `config/site.json`. Each entry has an `imageUrl` and bilingual `caption`. Add, remove, or reorder entries to change the public gallery.
...
- Touch swipe, previous/next controls, and dots for product images
...
1. Copy this whole `v3-gallery-calendar` folder into a new project/version folder.
