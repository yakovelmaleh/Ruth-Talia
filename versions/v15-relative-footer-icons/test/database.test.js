...
test("generates 60-minute Sunday through Thursday slots and supports closing", () => {
  ...
  try {
    ...
    const first = slots[0];
    store.toggleSlot(first.id, "Private atelier event");
    assert.equal(store.listAvailableSlots().some(slot => slot.id === first.id), false);
    assert.equal(store.listSlots({ status: "closed" })[0].close_reason, "Private atelier event");
    store.toggleSlot(first.id);
    assert.equal(store.listAvailableSlots().some(slot => slot.id === first.id), true);
    assert.equal(store.listSlots({ status: "open" })[0].close_reason, "");
  } finally {
    clean();
  }
});

test("closing an appointment slot allows an optional reason", () => {
  const { root, clean } = fixture();
  try {
    const store = createDatabase(root);
    const slotId = store.createSlot({date:"2099-01-02", time:"10:30", durationMinutes:60, location:"atelier"});
    assert.equal(store.toggleSlot(slotId), true);
    assert.equal(store.listSlots({ status: "closed" })[0].close_reason, "");
  } finally {
    clean();
  }
});

test("analytics counts visitors, devices, sources, and countries", () => {
  const { root, clean } = fixture();
  try {
    const analytics = createAnalyticsFeature(root);
    analytics.recordVisit({
      visitorId: "visitor-1", path: "/?lang=en", language: "en", screenWidth: 390,
      userAgent: "Mobile Safari", referrer: "https://instagram.com/ruthtalia", country: "IL",
      timezone: "Asia/Jerusalem"
    });
    analytics.recordVisit({
      visitorId: "visitor-1", path: "/?lang=he", language: "he", screenWidth: 390,
      userAgent: "Mobile Safari", referrer: "", country: "IL", timezone: "Asia/Jerusalem"
    });
    const dashboard = analytics.dashboard({from:"2000-01-01", to:"2099-12-31", device:"mobile"});
    assert.equal(dashboard.totals.views, 2);
    assert.equal(dashboard.totals.visitors, 1);
    assert.equal(dashboard.devices[0].label, "mobile");
    assert.ok(dashboard.sources.some(item => item.label === "instagram.com"));
    assert.equal(dashboard.countries[0].label, "IL");
  } finally {
    clean();
  }
});
