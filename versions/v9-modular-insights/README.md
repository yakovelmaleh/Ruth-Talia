...
Version 9 adds four more dresses, a dedicated transparent image button so every selling image reliably opens its details, improved desktop appointment proportions, narrow-device containment, compact calendar filters, closure reasons, and a reusable first-party analytics feature.
...
```bash
cd /Users/yakovelmaleh/ruth-talia-couture-website/versions/v9-modular-insights
npm install
...
Open `/admin#calendar` and filter by date and status. The table displays 40 rows by default, with options for 20, 40, or 80 rows. Closing an unbooked time requires a short reason, which remains visible in the new **Reason closed** column. Select **Reopen** to make it public again and clear the closure reason. When a client books a time, it disappears from public availability and can no longer be changed.
...
1. Copy this whole `v9-modular-insights` folder into a new project/version folder.
