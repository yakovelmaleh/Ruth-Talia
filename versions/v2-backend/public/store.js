(() => {
  ...
  document.querySelectorAll("[data-product-open]").forEach(button => button.addEventListener("click", () => {
    ...
    productDialog.querySelector("[data-dialog-image]").src = selectedProduct.imageUrl;
  ...
  document.querySelector("[data-appointment-form]").addEventListener("submit", async event => {
    ...
    const status = form.querySelector(".form-status");
    try {
      await request("/api/appointments", {method:"POST", body:JSON.stringify(Object.fromEntries(new FormData(form)))});
