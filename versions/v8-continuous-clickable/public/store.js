(() => {
  ...
  class InfiniteCarousel {
    ...
    startDrag(event) {
      ...
      if (event.target.closest("button") || this.animating) return;
      this.dragging = true;
    ...
    drag(event) {
      ...
      if (Math.abs(this.dragX) > 8) this.suppressClickUntil = Date.now() + 300;
  ...
  document.querySelectorAll("[data-card-carousel]").forEach(container => {
    ...
    container.addEventListener("click", event => {
      if (event.target.closest("button") || Date.now() < carousel.suppressClickUntil) return;
